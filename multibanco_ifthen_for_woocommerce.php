...
Email enviado automaticamente do plugin WordPress "Multibanco (IfthenPay gateway) for WooCommerce"';
						$headers=array(
				...
				function callback() {
					...
					if (isset($_GET['chave'])
						...
						if ($arguments_ok) { // Isto deve ser separado em vários IFs para melhor se identificar o erro
							...
							if (trim($this->debug_email)!='') wp_mail(trim($this->debug_email), $this->id.' - Error: Callback with argument errors', 'Callback ( '.$_SERVER['HTTP_HOST'].' '.$_SERVER['REQUEST_URI'].' ) with argument errors from '.$_SERVER['REMOTE_ADDR'].$arguments_error);
